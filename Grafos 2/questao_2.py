...
near_friends = []
for vertex in graph:
    if vertex.getId() == "Mussum": continue

    mussum = graph.getVertex("Mussum")
    if vertex in mussum.connectedTo: continue

    count = 0
    for crush in vertex.connectedTo:
        if crush in mussum.connectedTo: count += 1

    if count >= 3: 
        near_friends.append(vertex.getId())

if near_friends: 
    for friend in sorted(near_friends): print(friend) 
else: print("Cacildis! Cade elis?")
